...
const ComplaintPost = ({ data }) => {
  ...
  const handleVote = async (complaintId, isDownvote) => {
    try {
      ...
      const singleComplaint = complaintData.find(
        (c) => c.intComplaintId === complaintId
      );
        ...
        <Card key={complaint.intComplaintId} className="filterStyle">
          <CardContent>
              ...
              <FlexBetween>
                <div style={{ display: "flex", alignItems: "center" }}>
                  ...
                  <span style={{ marginLeft: "10px" }}>
                    {complaint.strFirstName} {complaint.strLastName}
                  </span>
                  <FlexBetween>
                ...
                <div className="status-box">
                  <Typography variant="body2">
                    الحالة العامة:{" "}
                    <StatusTracker currentStage={complaint.strStatusEn} />{" "}
                  </Typography>
                </div>
              </FlexBetween>
            </Typography>
            ...
            <Typography
              ...
              sx={{ width: "85%", display: "grid", margin: "auto" }}
            >
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
              <img
                src={
                  complaint.imageData
                    ? `data:image/jpg;base64,${complaint.imageData}`
              ...
              <FlexBetween>
                <div style={{ paddingRight: "36px", marginTop: "13px" }}>
                  <IconButton
                    ...
                    className={`${
                      complaint.intVoted === 1 ? "upvote_green" : ""
                    } ${complaint.intVoted === -1 ? "disabled" : ""}`}
                    onClick={() => handleVote(complaint.intComplaintId, false)}
                  >
                    {complaint.isDownVote === false ? (
                      <ThumbUp />
                    ) : (
                      <ThumbUpOutlined />
                    )}
                  ...
                  <IconButton
                    onClick={() => setWatch(complaint.intComplaintId)}
                  >
                ...
                <Typography
                  variant="h4"
                  sx={{ pl: 8, paddingTop: 3, color: "#18AAC9" }}
                >
                  {" "}
                  ش. المدينة المنورة, عمان
                </Typography>
              </FlexBetween>
            </div>
          </CardContent>
